...
# 判断 Python 版本是否大于 3.9
$pythonVersion = python --version 2>&1 | % { $_ -match 'Python (\d+\.\d+)' | Out-Null; $Matches[1] }

if ($pythonVersion -gt '3.9') {
    Write-Host "Python version is $pythonVersion, has been satisfied ..."
    # 在这里添加需要执行的命令
    ...
    Write-Host "Your Python version is  $pythonVersion and it is TOO LOW ,need >=3.9!!!..."
